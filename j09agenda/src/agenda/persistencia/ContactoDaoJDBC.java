	...
	public void insertar(Contacto c) {
		...
		try(Connection con = ds.getConnection()) {
			...
			if(filas == 1) {
				...
				PreparedStatement psId = con.prepareStatement("select LAST_INSERT_ID()");//esta consulta es propia de myqsl
				...
				//cantidad de telefonos guardados
				int cantTel = 0;
		...
		while(rs.next()) {
			...
			sql ="select correo from correos where fk_contacto=?";
					...
					while(rsCorreos.next()) {
						c.addTelefonos(rsCorreos.getString("correo"));
					}
